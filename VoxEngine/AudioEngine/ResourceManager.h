...
namespace AudioEngine
{
	...
	class ResourceManager
	{
		...
		SoundId mNextSoundId = 0; // TODO: Use free lists instead of incrementing IDs forever
	public:
		// Loads a sound file and returns its SoundId
		std::optional<SoundId> loadSound(FileExtension ext, const std::filesystem::path& path);

		// Overload that infers file extension from the path
		std::optional<SoundId> loadSound(const std::filesystem::path& path);
